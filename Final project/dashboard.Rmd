...
gdp_mean_sd_dF <- knowledge_traditinoal_dF %>%
  group_by(TableName) %>%
  summarise("Country Mean GDP" = mean(GDP, na.rm=TRUE),
            "Country SD GDP" = sd(GDP, na.rm=TRUE)
            )

knowledge_traditinoal_dF <- merge(knowledge_traditinoal_dF, gdp_mean_sd_dF, by.x = "TableName", by.y = "TableName", all.y = T) %>%
...
knowledge_traditinoal_SD_25to50percent_dF <- knowledge_traditinoal_dF %>%
  filter(.,`Country SD GDP in percent` > 25 & `Country SD GDP in percent` <=50) %>%
  arrange(TableName, year)

knowledge_traditinoal_SD_50to75percent_dF <- knowledge_traditinoal_dF %>%
  filter(.,`Country SD GDP in percent` > 50 & `Country SD GDP in percent` <=75) %>%
  arrange(TableName, year)
...
knowledge_traditinoal_SD_1to25percent_dF
```
...
### SD >25 and upto 50 percent

```{r fig.width=15, echo = F}

ggplot(knowledge_traditinoal_SD_25to50percent_dF, aes(x=factor(year), colour=TableName, group = TableName)) +
  geom_point(aes(y = `Knowledge based Percent of GDP`)) + 
  geom_line(aes(y = `Knowledge based Percent of GDP`)) +
  theme(axis.text.x = element_text(size=10, angle=90)) +
  theme(axis.text.y = element_text(size=10, angle=90)) +
...
### SD >50 upto 75 percent

```{r fig.width=15, echo = F}

ggplot(knowledge_traditinoal_SD_50to75percent_dF, aes(x=factor(year), colour=TableName, group = TableName)) +
  geom_point(aes(y = `Knowledge based Percent of GDP`)) + 
  geom_line(aes(y = `Knowledge based Percent of GDP`)) +
  theme(axis.text.x = element_text(size=10, angle=90)) +
  theme(axis.text.y = element_text(size=10, angle=90)) +
